<template>
  <Modal title="Start new game" :visible="configuration" :showClose="config.games.value > 0">
    ...
    <p>
      <label for="initial-armies">CPU initial number of player armies (<span class="accent">{{ initialArmies }}</span>):</label>
    ...
    <p>
      <label for="max-armies">Maximum number of armies in one territory (<span class="accent">{{ maxArmies }}</span>):</label>
